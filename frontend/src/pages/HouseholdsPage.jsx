...
const isTrulyEmpty = (search, active, totalAll) =>
  !search && active === "true" && totalAll === 0;

function emptyMessage(search, active, totalAll) {
  if (search) return "No households match that search.";
  if (isTrulyEmpty(search, active, totalAll))
    return "No households have been registered yet.";
      ...
      <div className="modal" onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit}>
          <label>
            ...
            <span className="hint">
              {" "}
              — the household is identified by this person
            </span>
          </label>
          <ResidentPicker
            value={head}
          ...
          <p className="muted small-note">
            Two households may share an address — you will get a notice, not an
            error.
          </p>
          ...
          <div className="actions">
            <button
              className="btn"
              type="submit"
              disabled={!head || !address.trim() || busy}
            >
...
function AddMemberPanel({ householdId, onDone, onCancel }) {
  ...
  async function submit(transfer) {
    ...
    try {
      ...
      if (
        err.status === 409 &&
        err.data?.household_id &&
        err.data.household_id !== householdId
      ) {
    ...
    <div className="card wide" style={{ maxWidth: "100%", marginBottom: 16 }}>
      ...
      {error && (
        <div className="alert error">
          ...
          {conflict && (
            <div className="reason-note">
              Moving them here will end their membership in household #
              {conflict.household_id} today. The old membership is kept as
              history.
            </div>
          )}
        </div>
          ...
          {currentHeadName ? (
            <>
              ...
              <label style={{ marginTop: 8, display: "block" }}>
                ...
                <select
                  value={demoteTo}
                  onChange={(e) => setDemoteTo(e.target.value)}
                >
                  {ASSIGNABLE_ROLES.map((r) => (
                    <option key={r} value={r}>
                      {r}
            ...
            <div className="reason-note">
              This household currently has no head.
            </div>
          )}
          <div className="actions">
            <button
...
function HouseholdDetail({ householdId, canManage, onBack, onChanged }) {
  ...
  async function endMembership(m) {
    if (
      !window.confirm(
        `End ${m.name}'s membership in this household? The record is kept as history.`,
      )
    )
      return;
    ...
    const ok = await run(`head-${m.membership_id}`, () =>
      authFetch(`/households/${householdId}/head`, {
        ...
        body: {
          ...
          demote_current_head_to: demoteTo,
        },
      }),
    );
  ...
  async function saveAddress(e) {
    ...
    const ok = await run("address", () =>
      authFetch(`/households/${householdId}`, {
        method: "PATCH",
  ...
  async function toggleActive() {
    ...
    await run("active", () =>
      authFetch(`/households/${householdId}`, {
        method: "PATCH",
        body: { is_active: !household.is_active },
      }),
    );
  }
  ...
  const visibleMembers = showEnded
    ? household.members
    : household.members.filter((m) => m.is_active);
    ...
    <>
      <div className="list-head">
        <h2>
          ...
          {household.head_name
            ? ` — Household of ${household.head_name}`
            : " — no head assigned"}
        </h2>
      ...
      {household.head_is_archived && (
        <div className="alert error">
          <strong>
            This household&apos;s head is an archived resident record.
          </strong>
          <div className="reason-note">
            The household is still on file, but its head is no longer on the
            active resident master list. Reassign the head to another member.
          </div>
        </div>
      )}
      {!household.head_name && household.is_active && (
        <div className="alert error">
          ...
          <div className="reason-note">
            Use “Make head” on a member to assign one.
          </div>
        </div>
      )}
      {!household.is_active && (
        <div className="alert">
          ...
          <div className="reason-note">
            Its memberships were ended when it was deactivated. Reactivating
            does not restore them — members must be re-added.
          </div>
        </div>
      )}
        ...
        <table className="data-table">
          <tbody>
            ...
            <tr>
              ...
              <td>
                <span
                  className={`badge ${household.is_active ? "status-claimed" : "status-cancelled"}`}
                >
      ...
      <div className="list-head" style={{ marginTop: 20 }}>
        ...
        <div className="head-actions">
          {endedCount > 0 && (
            ...
            >
              {showEnded
                ? `Hide past members (${endedCount})`
                : `Show past members (${endedCount})`}
            </button>
          )}
                ...
                <Fragment key={m.membership_id}>
                  <tr className={m.is_active ? undefined : "row-ended"}>
                    <td>
                      ...
                      {m.resident_is_archived && (
                        <span className="muted"> · archived record</span>
                      )}
                      {!m.is_active && (
                        <span className="muted">
                          {" "}
                          · ended {dateOnly(m.date_ended)}
                        </span>
                      )}
                    </td>
                    <td>
    ...
    <>
      ...
      {data.residents.length === 0 ? (
        <div className="empty">
          <p>
            {search
              ? "No unassigned residents match that search."
              : "Every active resident belongs to a household."}
          </p>
        </div>
      ) : (
        <>
